...
def injector1(C_d, m, OF, rho_ox, rho_f, mu_prop, sig_prop, rho_prop):
    ...
    d_f = (0.0135 +0.281)*0.0254/2.0         # 3.74 mm # http://libgen.rs/book/index.php?md5=3D236B9BDD4070690CA83952058D9A1F p.113
    ...
    def Discharge(A_s, A_I):
        K_comp = 1.0
        zeta_c = K_comp*0.5*(1.0 - A_s/A_I)**(3.0/4.0)    # For sudden contraction
        zeta_e = (1.0 - A_s/A_I)**2.0                 # For sudden expansion
        zeta = zeta_c + zeta_e
        C_d = (1.0/zeta)**0.5
        return C_d, zeta
    ...
    if InjType == 'unlike': #liquid - liquid   
        ...
        d_j = d_f
        v_j = (v_iox + v_if)/2.0
        D_f = D_o = (1.6e5*(v_j*3.28084)**(-1.0)*(p_center/p_j)**(-0.1)*(d_j*39.3701)**0.57*K_prop)*0.0254e-6
        # print(D_f)
