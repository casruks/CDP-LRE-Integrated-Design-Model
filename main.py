...
def Main(Thrust, Thrust_time, Pamb):
    ...
    while abs(p_new - p_old) / p_new > default.pres_tol:
        p_old = p_new
        # Compute nozzle (1)
        m, Tc, O_F, At, eps, Isp, rho_c, cp_c, mu_c, k_c, Pr_c = Nz_1.Nozzle_loop_1(
            p_new / 100000, Thrust, Pamb / 100000, prop, default
        )
        ...
        Cd = 0.7
        v_iox, v_if, D_f, D_o = Inj.injector1(
            Cd, m, O_F, prop.o_dens, prop.f_dens_l, mu_prop, sig_prop, rho_prop
        )
        # Compute chamber - needs Chamber temperature + oxider to fuel ratio from previous functions (Tc and of)
        h_comb, Dc, ThicknessChamber, Chamber_L, Re_c = Comb.CombustionChamber(
            p_new, At, prop, Mt.Rhenium, default.SF, inj_vel, D_o, Tc, O_F, bool
        )

        # COmpute nozzle (2)
        t_noz, x_noz, y_noz, Tw_ad_noz, h_c_noz, P_noz, T_noz, Re_t = Nz_2.Nozzle_loop(
            p_new / 100000,
            Tc,
            prop,
            Mt.Rhenium,
        ...
        Tf_cool, dptcool_c = regCool.Run_for_Toperating0D(
            Tc,
            h_comb,
            ThicknessChamber,
            prop,
            Mt.Rhenium,
            Chamber_L * default.Dr,
            Tf_cool,
            m / (1 + O_F),
            Chamber_L,
        )
        ...
        # Cmpute Injector (2)
        p_new, dp_ox, dp_f = Inj.injector2(
            v_iox, v_if, D_f, D_o, ptinj, Cd, prop.o_dens, prop.f_dens_l
        )
        print(p_new)
