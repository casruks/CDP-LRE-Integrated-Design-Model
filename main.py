...
def Main(d: aux.Data):
    ...
    while abs(p_new - p_old) / p_new > default.pres_tol:
        ...
        (
            ...
            er,
            wr,
        ) = Inj.injector1(default, prop, p_new, d.m_nozz, d.O_F)
        ...
        d.ptinj, d.W_Opump, d.W_Fpump, d.W_turb, error = Turbo.TurboM(
            default, prop, d.O_F, d.Pa, Tf_cool, dp_cool, d.m_nozz
        )
        if error:
            return False

        # Compute Injector (2)
        p_new, dp_ox, dp_f, er, wr = Inj.injector2(
            default, prop, d.v_iox, d.v_if, d.ptinj, d.eta_s
        )
    ...
    Ign_propellant_mass, Ign_fuel_mass, Ign_ox_mass, wr_ign = Ign.Igniters(
        d.m_nozz, prop, default, d.Tc, d.O_F, default.type
    )
    # outputs
