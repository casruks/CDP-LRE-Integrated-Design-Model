...
def Main(d: aux.Data, com: GUI.Communicate):
    ...
    while abs(p_new - p_old) / p_new > default.pres_tol:
        ...
        (
            ...
            er,
            wr,
        ) = Inj.injector1(default, prop, p_new, d[-1].m_nozz, d[-1].O_F)
        ...
        (
            d[-1].h_comb,
            d[-1].Dc,
            d[-1].ThicknessChamber,
            d[-1].Chamber_L,
            d[-1].Re_c,
            wr_comb,
            er_comb,
        ) = Comb.CombustionChamber(
            p_new,
            d[-1].At,
            prop,
            Ms.Rhenium,
        ...
        (
            ...
            Ms.Rhenium,
            default.default_coating,
            np.array([d[-1].ThicknessChamber for x in range(len(t_noz))]),
            t_noz,
            np.array([default.default_coating_thickness for x in range(len(t_noz))]),
            y_noz,
            d[-1].Dc / 2,
            error_out_cool,
        )
        ...
        (
            ...
            default, prop, d[-1].O_F, d[-1].Pa, Tf_cool, dp_cool, d[-1].m_nozz
        )
        ...
        # Compute Injector (2)
        p_new, dp_ox, dp_f, er_in, wr_in = Inj.injector2(
            default, prop, d[-1].v_iox, d[-1].v_if, d[-1].ptinj, d[-1].eta_s
        )
    ...
    Ign_propellant_mass, Ign_fuel_mass, Ign_ox_mass, wr_ign = Ign.Igniters(
        d[-1].m_nozz, prop, default, d[-1].Tc, d[-1].O_F, default.type
    )
    # outputs
    ...
    Reliability = Rel.Reliability(
        default, d[-1].time, d[-1].Thrust, d[-1].Thrust, default.val
    )

    # Compute Mass:
    ChamberMass = Comb.CombustionChamber(
        p_new,
        d[-1].At,
        prop,
        Ms.D6AC_Steel,
    ...
    Mass = (
        ...
        + Ms.Mass(
            p_new,
            aux.Default.noz_mat_select,
            aux.Default.valv_mat_select,
            d[-1].Eps,
            d[-1].A_t,
            d[-1].m_nozz,
            aux.Default.Safety_factor,
            x_noz,
            y_noz,
            t_noz,
            prop.o_dens,
    ...
    Cost = Ms.Cost(
        Mass,
        aux.Default.tech_ready,
        aux.Default.exp_factor,
        Reliability,
        aux.Defualt.learn_factor,
        aux.Defualt.cycle_types,
    )
    # Outputs:
    ##Cost of the engine
