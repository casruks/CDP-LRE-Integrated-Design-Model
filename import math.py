...
def plot_nozzle(ax, title, Rt, angles, contour):
    ...
    ax.annotate(
        "", [xcenter, ycenter], [xarch, yarch], arrowprops=dict(lw=0.5, arrowstyle="<-")
    )
...
def draw_angle_arc(ax, theta_n, origin, degree_symbol=r"$\theta$"):
    ...
    ax.text(
        startx + 0.5,
        starty + 0.5,
        degree_symbol + " = " + str(round(theta_n, 1)) + "\u00b0",
    )
    return
...
if __name__ == "__main__":
    ...
    aratio = 77.5  # Ae / At
    throat_radius = 0.26  # {'radius_throat': 40, 'radius_exit': 210}
